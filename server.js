...
import { loadScripts, parameterize } from './src/cdp-scripts/loader.js';
...
// === CDP Scripts (loaded from src/cdp-scripts/ at startup) ===
...
// ─────────────────────────────────────────────
// Snapshot Capture (scripts loaded from src/cdp-scripts/)
// ─────────────────────────────────────────────

// Compute a hash over all snapshot fields for change detection.
// Centralized here — was previously duplicated 5× (poll loop + 4 burst captures).
...
// Fire rapid re-captures after portal-opening clicks.
// Centralized here — was previously duplicated 4× in /click handler branches.
function burstCapture(delays) {
...
async function captureSnapshot() {
  try {
    let result = await evaluateInBrowser(scripts.capture);
    // When capture returns null (e.g. Scheduled Tasks page has no chat container),
    ...
    try {
      result.runningTasksHtml = await evaluateAcrossContexts(scripts.captureRunningTasks);
    } catch (e) {
    ...
    try {
      result.scheduledTasksHtml = await evaluateAcrossContexts(scripts.captureScheduledTasks);
    } catch (e) {
    ...
    if (result.scheduledTasksHtml) {
      try {
        result.scheduledTasksDialogHtml = await evaluateAcrossContexts(scripts.captureScheduledTasksDialog);
      } catch (e) {
      ...
      if (!result.dropdownHtml) {
        try {
          result.dropdownHtml = await evaluateInBrowser(scripts.captureDropdown);
        } catch (e) {
      ...
      if (!result.dropdownHtml) {
        try {
          result.dropdownHtml = await evaluateAcrossContexts(scripts.captureKebabMenu);
        } catch (e) {
...
async function injectMessage(text, opts = {}) {
  ...
  const script = parameterize(scripts.inject, {
    __TEXT__: JSON.stringify(text),
    __APPEND_MODE__: String(appendMode),
  });
...
async function stopGeneration() {
  return await evaluateInBrowser(scripts.stop);
}
...
app.get('/right-sidebar', async (req, res) => {
  try {
    let html = await evaluateInBrowser(scripts.captureRightSidebar);
    if (html) {
    ...
    // Re-try capture
    html = await evaluateInBrowser(scripts.captureRightSidebar);
...
app.get('/proxy-image', async (req, res) => {
  ...
  try {
    const script = parameterize(scripts.proxyImage, {
      __TARGET_SRC__: JSON.stringify(src),
    });
...
app.post('/expand-left-sidebar', async (req, res) => {
  ...
  try {
    const result = await evaluateInBrowser(scripts.expandLeftSidebar);
...
app.post('/copy-response', async (req, res) => {
  ...
  try {
    const script = parameterize(scripts.copyResponse, {
      __CLICK_ID__: JSON.stringify(String(clickId)),
    });
...
app.post('/dismiss-scheduled-tasks', async (req, res) => {
  ...
  try {
    const result = await evaluateAcrossContexts(scripts.dismissScheduledTasks);
...
app.post('/dismiss-settings', async (req, res) => {
  ...
  try {
    const result = await evaluateInBrowser(scripts.dismissSettings);
...
app.post('/click', async (req, res) => {
  ...
  try {
    ...
    if (String(clickId).startsWith('task:')) {
      ...
      const script = parameterize(scripts.clickTask, { __TASK_IDX__: String(taskIdx) });
      const result = await evaluateAcrossContexts(script);
    ...
    if (String(clickId).startsWith('sched:')) {
      ...
      const script = parameterize(scripts.clickSched, { __SCHED_IDX__: String(schedIdx) });
      const result = await evaluateAcrossContexts(script);
    ...
    if (String(clickId).startsWith('scheddlg:')) {
      ...
      // scheddlg:100+ → body-level portal options (listbox / popover / context menu)
      if (dlgIdx >= 100) {
        const optIdx = dlgIdx - 100;
        const script = parameterize(scripts.clickScheddlgPortal, { __OPT_IDX__: String(optIdx) });
        // Try preferred context first
        let result = await evaluateInBrowser(script);
        // If not found, try across all contexts (kebab menu in isolated context)
        if (!result) {
        ...
        if (result?.ok) {
          burstCapture([150, 400, 800]);
        }
        return;
      }

      // scheddlg:0-99 → elements inside the z-[2550] dialog overlay
      const script = parameterize(scripts.clickScheddlg, {
        __DLG_IDX__: String(dlgIdx),
        __LABEL__: JSON.stringify(label || ''),
      });
      const result = await evaluateAcrossContexts(script);
    ...
    // All other click sources: chat, left, right, dropdown, dialog, settings, perm, env, model, project
    const clickScript = parameterize(scripts.click, {
      __CLICK_ID__: JSON.stringify(String(clickId)),
      __LABEL__: JSON.stringify(label || ''),
    });
    ...
    // After portal-opening clicks, schedule rapid re-captures
    if (result?.ok) {
...
app.post('/type-text', async (req, res) => {
  ...
  const script = parameterize(scripts.typeText, {
    __PLACEHOLDER__: JSON.stringify(placeholder || ''),
    __CLICK_ID__: JSON.stringify(clickId || ''),
    __TEXT__: JSON.stringify(text),
  });
...
app.post('/upload', upload.single('image'), async (req, res) => {
  ...
  try {
    const script = parameterize(scripts.upload, {
      __BASE64__: JSON.stringify(base64),
      __MIMETYPE__: JSON.stringify(mimetype),
      __FILENAME__: JSON.stringify(fileName),
    });
    const result = await evaluateInBrowser(script);
...
app.get('/discover', async (req, res) => {
  ...
  try {
    const result = await evaluateInBrowser(scripts.discover);
