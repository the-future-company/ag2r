...
- **Right sidebar selector is fragile.** The AG right panel is found via `data-tab-id` buttons and `toggle-aux-sidebar` testid in `RIGHT_SIDEBAR_SCRIPT`. There are no stable container IDs. If AG's layout changes, the sidebar capture may fail silently (returns null). Use `GET /discover` to debug.
...
- **Right sidebar close syncs AG2R → AG.** When the user closes AG2R's sidebar, `POST /close-sidebar` clicks AG's `toggle-aux-sidebar` button (fire-and-forget). This keeps both UIs in sync so re-clicking an artifact produces a detectable tab change. Without this sync, AG's sidebar stays open and re-clicks are invisible to the tab signature comparison.
...
- **Settings overlay uses `z-[5000]`.** The settings modal overlay selector is `#root .fixed.inset-0[class*="z-[5000]"]` — used in capture, click-main, and dismiss-settings. AG changed this from `z-[2550]` around June 2026. If settings capture breaks again, inspect `#root .fixed.inset-0` elements' z-index classes.
- **Settings dismiss uses backdrop click.** `dismiss-settings` in `server.js` clicks the settings modal's backdrop overlay (`.bg-black\/80`) — targetted via the `z-[5000]` overlay — instead of a Go Back button, ensuring settings close in one action regardless of which tab was visited.
